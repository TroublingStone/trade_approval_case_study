...
class TradeDetails:
    ...
    def _validate(self) -> None:
        if not self.trading_entity.strip():
            raise EmptyPartyNameError("trading_entity")
        if not self.counterparty.strip():
            raise EmptyPartyNameError("counterparty")
        ...
        if not self.notional_amount.is_finite():
            raise NonFiniteNotionalAmountError(self.notional_amount)
